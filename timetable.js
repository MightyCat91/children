jQuery(function ($) {
    ...
    function generateMobileTable () {
        $.each(days, function (index, value) {
            ...
            $('.table__wrapper.active').find('.' + value + ' .event__container').map(function (indx, element) {
                var subjectName = $(element).find('.subject__name').text(),
                    subjectAge = $(element).find('.subject__age').text(),
                ...
                newSubject.find('.small__subject__age').text(subjectAge);
